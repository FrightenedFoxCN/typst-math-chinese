...
- `def` 等数学模块均继承自 `make-boxed-theorem` 和 `make-companion-theorem`，可选地提供标号后的附注. 可以引用这些函数和 `thmbox` 自己新增模块，和 `ctheorems` 包兼容.
- 数学模块的引用直接继承块中的标题格式，链接模式更显眼.
...
- [x] 处理 listings 产生的问题；
- [ ] 增加 property 块；
- [ ] 修理附录编号的问题：可能需要一个大传参逻辑重构；
- [ ] 页眉、更多格式化页（标题等）.
- [ ] 丰富字体支持：在当前的 typst 版本处理兼容性尚有问题